...
from app.core.services.jwt_service import create_token_claims
from src.app.core.services import session_service
from src.app.core.services.session_service import (
    AuthSession,
    UserSession,
    create_auth_session,
    create_user_session,
...
class TestSessionService:
    ...
    @pytest.mark.asyncio
    async def test_auth_session_lifecycle(self):
        ...
        # Retrieve session
        auth_session = await session_service.get_auth_session(session_id)
        ...
        assert await session_service.get_auth_session(session_id) is None

    @pytest.mark.asyncio
    async def test_auth_session_expiry(self):
        ...
        auth_session = await session_service.get_auth_session(session_id)
        assert auth_session is not None
        await session_service.update_auth_session(
            session_id=session_id, extension_seconds=-10
        )

        # Should return None and clean up
        result = await session_service.get_auth_session(session_id)
    ...
    async def test_user_session_lifecycle(self):
        ...
        # Create session
        session_id = await session_service.create_user_session(
        ...
        assert await session_service.get_user_session(session_id) is None

    @pytest.mark.asyncio
    async def test_user_session_updates_last_accessed(self):
    ...
    @pytest.mark.asyncio
    async def test_auth_session_cleanup_on_expiry_check(self):
        ...
        for session_id in session_ids:
            assert await session_service.get_auth_session(session_id) is not None

        # Expire the middle session
        middle_session = await session_service.get_auth_session(session_ids[1])
        ...
        assert await session_service.get_auth_session(session_ids[1]) is None

        # Other sessions should remain
        assert await session_service.get_auth_session(session_ids[0]) is not None
        assert await session_service.get_auth_session(session_ids[2]) is not None

    @pytest.mark.asyncio
    async def test_user_session_expiry_handling(self):
        ...
        result = await session_service.get_user_session(session_id)
        assert result is None
    ...
    async def test_session_isolation(self):
        ...
        auth_id1 = await session_service.create_auth_session(
            nonce=generate_nonce(),
            client_fingerprint_hash="test_fingerprint",
        ...
        auth_id2 = await session_service.create_auth_session(
            nonce=generate_nonce(),
            client_fingerprint_hash="test_fingerprint",
        ...
        user_session_id1 = await session_service.create_user_session(
            client_fingerprint="test_fingerprint",
            user_id=user_id1,
            provider="google",
        ...
        user_session_id2 = await session_service.create_user_session(
            client_fingerprint="test_fingerprint",
            user_id=user_id2,
            provider="github",
        ...
        auth1 = await session_service.get_auth_session(auth_id1)
        auth2 = await session_service.get_auth_session(auth_id2)
        ...
        user1 = await session_service.get_user_session(user_session_id1)
        user2 = await session_service.get_user_session(user_session_id2)
        ...
        assert await session_service.get_auth_session(user_session_id1) is None
        assert await session_service.get_user_session(auth_id1) is None

    @pytest.mark.asyncio
    async def test_session_id_collision_resistance(self):
    ...
    async def test_refresh_user_session_success(self):
        ...
        with patch(
            ...
            new_session = await session_service.get_user_session(new_session_id)
    ...
    async def test_session_memory_management(self):
        ...
        initial_auth_count = len(await session_service.list_auth_sessions())
        initial_user_count = len(await session_service.list_user_sessions())

        # Create and immediately delete many sessions
        for i in range(50):
            # Auth sessions
            auth_id = await session_service.create_auth_session(
                nonce=generate_nonce(),
                client_fingerprint_hash="test_fingerprint",
                pkce_verifier=f"verifier-{i}",
                state=f"state-{i}",
            ...
            await session_service.delete_auth_session(auth_id)
            ...
            await session_service.delete_user_session(user_session_id)

        # Memory should be cleaned up
        final_auth_count = len(await session_service.list_auth_sessions())
        final_user_count = len(await session_service.list_user_sessions())
    ...
    async def test_concurrent_session_operations(self):
        ...
        await session_service.delete_user_session(session_ids[1])

        # Verify other sessions are unaffected
        assert await session_service.get_user_session(session_ids[0]) is not None
        assert await session_service.get_user_session(session_ids[1]) is None
        assert await session_service.get_user_session(session_ids[2]) is not None

        # Clean up
        await session_service.delete_user_session(session_ids[0])
        await session_service.delete_user_session(session_ids[2])
...
class TestSessionServiceNew:
    ...
    @pytest.mark.asyncio
    async def test_create_auth_session(self):
        """Test creating auth session with security features."""
        with patch(
            "src.app.core.services.session_service.get_session_storage"
        ) as mock_storage:
    ...
    @pytest.mark.asyncio
    async def test_validate_auth_session(self, test_auth_session: AuthSession):
        ...
        with patch(
            ...
            # Mock get_auth_session to return our test session
            mock_get_auth.return_value = test_auth_session

            # Test successful validation
            result = await validate_auth_session(
                session_id="test_session",
                state=test_auth_session.state,
                client_fingerprint_hash=test_auth_session.client_fingerprint_hash,
            )

            assert result is not None
            assert result.state == test_auth_session.state
            assert result.id == test_auth_session.id

            # Verify get_auth_session was called
            mock_get_auth.assert_called_once_with("test_session")

    @pytest.mark.asyncio
    async def test_validate_auth_session_failures(self, test_auth_session: AuthSession):
        ...
        with (
            ...
            patch(
                "src.app.core.services.session_service.delete_auth_session"
            ) as mock_delete,
        ):
            from src.app.core.models.session import AuthSession
            from src.app.core.security import hash_client_fingerprint
            ...
            result = await validate_auth_session("nonexistent", "state", "fingerprint")
            assert result is None
    ...
    def test_csrf_token_generation(self):
        ...
        # Validate token
        is_valid = validate_csrf_token_for_session(session_id, csrf_token)
        assert is_valid is True

        # Test invalid token
        is_valid = validate_csrf_token_for_session(session_id, "invalid_token")
        assert is_valid is False

        # Test None token
        is_valid = validate_csrf_token_for_session(session_id, None)
        assert is_valid is False
