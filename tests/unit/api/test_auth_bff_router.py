...
class TestBFFRouter:
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    ...
                    assert "auth_session_id=auth-session-123" in response.headers.get(
                        "set-cookie", ""
                    )
    ...
    def test_callback_success(self, test_client_with_mocks):
        ...
        with patch(
            ...
            with patch(
                "src.core.services.session_service.delete_auth_session"
            ) as mock_delete_auth:
                with patch(
                    ...
                    with patch(
                        ...
                        with patch(
                            ...
                            with patch(
                                ...
                                mock_create_user_session.return_value = (
                                    "user-session-789"
                                )

                                # Create test client with auth session cookie
                                test_client_with_mocks.cookies = {
                                ...
                                mock_delete_auth.assert_called_once_with(
                                    "auth-session-123"
                                )
    ...
    def test_get_auth_state_authenticated(self, test_client_with_mocks):
        ...
        with patch(
            "src.core.services.session_service.generate_csrf_token"
        ) as mock_csrf:
            mock_user = User(
            ...
            def mock_get_optional_session_user():
                return mock_user

            app.dependency_overrides[get_optional_session_user] = (
                mock_get_optional_session_user
            )
            ...
            try:
                ...
                assert result["user"]["id"] == "12345678-1234-5678-9abc-123456789012"
                assert result["user"]["email"] == "test@example.com"
                assert result["csrf_token"] == "csrf-token-456"
            finally:
                # Clean up dependency override
                app.dependency_overrides.pop(get_optional_session_user, None)
    ...
    def test_refresh_session_success(self, test_client_with_mocks):
        ...
        with patch(
            "src.core.services.session_service.refresh_user_session"
        ) as mock_refresh:
