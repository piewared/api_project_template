...
from src.runtime.config import ApplicationConfig, get_config, with_context
...
class TestOIDCClientService:
    ...
    async def test_exchange_code_for_tokens_success(
        self, mock_oidc_provider, mock_httpx_response
    ):
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            tmp_config = ApplicationConfig()
            tmp_config.oidc_providers["test"] = mock_oidc_provider

            with with_context(config_override=tmp_config):
    ...
    async def test_get_user_claims_from_id_token(
        self, mock_oidc_provider, mock_user_claims
    ):
    ...
    async def test_get_user_claims_from_userinfo_endpoint(
        self, mock_oidc_provider, mock_user_claims, mock_httpx_response
    ):
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            with with_context(config_override=temp_config):
                result = await oidc_client_service.refresh_access_token(
                    refresh_token="old-refresh-token", provider="test"
                )
